...
"""
## 概要 ##
insert_zabbix_user.py
1.SFTPコマンドを使用し、ZABBIXユーザー登録ファイルを読み込む。
...
# ZABBIXユーザー登録ファイル
CSV_FILE_PATH = '/usr/lib/zabbix/alertscripts/addownload.csv'
...
# ZABBIXユーザー登録ファイル(LDAPサーバ)
LDAPSVR_CSV_FILE_PATH = 'addownload.csv'
...
def copy_file_by_sftp():
    """
    SFTPコマンドを使用して、ZABBIXユーザー登録ファイルをコピーする。
    """
    # SFTPコマンド定義
    sftp = pexpect.spawn('sftp -P 6022 %s@%s:%s %s' % (LDAPSVR['user'], LDAPSVR['ip'], LDAPSVR_CSV_FILE_PATH, CSV_FILE_PATH))
    # パスワード入力補助返却
    returncode = sftp.expect([b'password:', pexpect.TIMEOUT, pexpect.EOF])
    if returncode > 0:
        if returncode == 1:
            LOGGER.info("%s: SFTP接続失敗：Timeout", LDAPSVR['ip'])
        elif returncode == 2:
            LOGGER.info("%s: SFTP接続失敗：Eof", LDAPSVR['ip'])
        sftp.close()
        sys.exit()
    # パスワード入力
    sftp.sendline(LDAPSVR['pswd'])
    # パスワード入力結果返却
    returncode = sftp.expect([r'.*100%.*', b'password:', pexpect.TIMEOUT, pexpect.EOF])
    if returncode > 0:
        if returncode == 1:
            LOGGER.info("%s: SFTP認証失敗：Password Auth Failed", LDAPSVR['ip'])
        elif returncode == 2:
            LOGGER.info("%s: SFTP認証失敗：Timeout", LDAPSVR['ip'])
        elif returncode == 3:
            LOGGER.info("%s: SFTP認証失敗：Eof", LDAPSVR['ip'])
        sftp.close()
        sys.exit()
    LOGGER.info("%s: SFTP成功", LDAPSVR['ip'])
    sftp.close()
...
def get_userid(api, alias):
    ...
    userid = ''
    error_message = ''
    response = api.request('user.get', {'output': 'userid', 'filter': {'alias': alias}})
    if 'result' in response:
        if response['result']:
...
def get_insert_data(api, params):
    ...
    data = {}
    data['alias'] = params['アカウント名']
    data['surname'] = params['姓']
    data['name'] = params['名']
    data['type'] = USER_TYPES[params['説明']]
    data['usrgrps'] = []
    # ユーザーグループID取得
...
def main():
    ...
    error_messages = []
    # ファイルをオープンする
    csvfile = open(CSV_FILE_PATH, 'r', encoding='cp932')
    # 1行ずつユーザー登録を行う
