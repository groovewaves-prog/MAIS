...
def patolamp_on(alarm_status, host):
    ...
    LOGGER.info("patolamp_on zabbix_host=%s, hostgroups=%s" % (host, hostgroups))
...
def main():
    ...
    # testファイルの有無を確認
    if FUNC.file_check('test') is True:
    ...
    # Primaryファイルの有無を確認
        ...
        # 20230630 暫定対応 >>
        LOGGER.info("get_action_no(%s, %s, %s),result=%s" % (insert_value[3], insert_value[4], insert_value[6], action_no) )
        LOGGER.info("=== insert_gw_evenst params begin ===")
        for i, val in enumerate(insert_value):
            LOGGER.info("[%d]:%s" % (i,val) )
        LOGGER.info("=== insert_gw_evenst params end ===") 
