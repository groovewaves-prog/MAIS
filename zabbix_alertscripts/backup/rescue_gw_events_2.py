#!/usr/bin/env python3
# -*- coding: utf-8 -*-

"""
## 概要 ##
# rescue_gw_events.py

# Zabbix 外部監視から実行
# 引数無しで「rescue_gw_events.py」を実行するスクリプト
#
#2017/08/20 大瀬良　初版作成
"""

## import ##
import datetime
#from datetime import datetime
import os
import sys
import logging
import logging.handlers
import re
import mysql.connector
import gwcommon as COM

## スクリプトのパス、ファイル名を取得
# __file__ = '/usr/lib/zabbix/alertscripts/rescue_gw_events.py'
SCRIPT_FILE = os.path.basename(__file__)

## log出力設定 ※rsyslogd に渡す設定
LOGGER = logging.getLogger(SCRIPT_FILE)
LOGGER.setLevel(logging.DEBUG)
HANDLER = logging.handlers.SysLogHandler(address=('localhost', 514), facility='local6')
FORMATTER = logging.Formatter('%(name)s[%(process)d]: %(levelname)s: [%(lineno)d]%(message)s')
HANDLER.formatter = FORMATTER
LOGGER.addHandler(HANDLER)

def file_check(filename):
    """
    /var/lib/baogw/primary or test ファイル有無をチェック
    """
    check_file = '/var/lib/baogw/' + filename
    return os.path.exists(check_file)

def connect_db():
    """
    databse へ接続 
    """
    try:
        connect_baogw = mysql.connector.connect(
            host=COM.MY_HOST,
            db=COM.MY_DB,
            user=COM.MY_USER,
            password=COM.MY_PASSWD
        )
        baogwcur = dbbaogw.cursor(dictionary=True) # カーソル
    except Exception as msg:
        LOGGER.critical('Faild to connect the DB, so exit.')
        LOGGER.info(msg)
        exit()
    return connect_baogw


def search_rescue_events():
    """
    検索用関数　結果をfound_recsにて返信
    """
    search_item = 'gw_event_id, update_time, detected_time, detected_host, customer_ci, customer_name,\
                     hostname, alarm_time, ci_name, device, alarm_status, summary'
    table_name = 'gw_rescue_events'
    search_condition = 'event_status=%s',COM.GW_EVENT_STATUS_NEW
    # search_baogw()をキックし、結果をfound_recsで受け取り    
    sql = 'SELECT %s FROM %s WHERE %s ' ,search_item,table_name,search_condition
    try:
        baogwcur.execute(sql)
        found_recs = baogwcur.fetchall()
        LOGGER.info('select成功:[%s] %s %s ', dbhost, table_name,search_condition)
    except mysql.connector.errors.IntegrityError as msg:
        LOGGER.critical('select失敗:[%s] %s %s ',  dbhost, table_name, search_condition)
        LOGGER.info('%s', msg)
    return found_recs


def insert_gw_events(found_rec):
    """
    INSERT用関数 結果をinsert_resultにて返信
    """
    table_name = 'gw_events'
    insert_item = 'update_time, detected_time, detected_host, customer_ci, customer_name,\
                  hostname, alarm_time, ci_name, device, alarm_status, summary'
    insert_value = make_insert_value(found_rec)

    sql = 'INSERT INTO %s %s VALUES %S',table_name ,insert_itme ,insert_value
    try:
        baogwcur.execute(sql)
        LOGGER.info('INSERT成功:[%s] %s %s', dbhost, table_name)
        insert_result = '0'
        # コミット
        dbbaogw.commit()
    except mysql.connector.errors.IntegrityError as msg:
        LOGGER.critical('INSERT失敗:[%s] %s %s', dbhost, table_name, insert_value)
        LOGGER.info('%s', msg)
    return insert_result
   


def delete_gw_rescue_events(delete_condition):
    """
    delete用関数　結果をdelete_resultにて返信
    """
    table_name = 'gw_rescue_events'
    sql = 'DELETE %s WHERE %S',table_name ,delete_condition
    try:
        baogwcur.execute(sql)
        LOGGER.info('delete成功:[%s] %s %s %s ', dbhost, table_name ,delete_condition)
        delete_result = '0'
        #コミット
        dbbaogw.commit()
    except mysql.connector.errors.IntegrityError as msg:
        LOGGER.critical('delete失敗:[%s] %s %s', dbhost, table_name, delete_condition)
        LOGGER.info('%s', msg)
    return delete_result

def make_insert_value(found_rec):
    """
    insert_value作成用関数 結果をinsert_valueにて返信 
    """
    insert_value = '( %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s )',found_rec['update_time'],\
    found_rec['detected_time'],found_rec['detected_host'],found_rec['customer_ci'],\
    found_rec['customer_name'],found_rec['hostname'],found_rec['alarm_time'],found_rec['ci_name'],\
    found_rec['device'],found_rec['alarm_status'],found_rec['summary']
    return insert_value

def search_events(found_rec):
    """
    gw_eventのチェック用関数　結果をcheck_resultにて返信
    """
    search_item = '*'
    table_name = 'gw_events'
    search_condition = 'alarm_time=%s and customer_name=%s and hostname=%s and ci_name=%s',\
    found_rec['alarm_time'],found_rec['customer_name'],found_rec['hostname'],found_rec['ci_name']
    sql = 'SELECT %s FROM %s WHERE %s ' ,search_item,table_name,search_condition
    try:
        baogwcur.execute(sql)
        search_recs = baogwcur.fetchall()
        check_result = len(search_recs)
    except mysql.connector.errors.IntegrityError as msg:
        LOGGER.critical('select失敗:[%s] %s %s', dbhost, table_name, insert_item)
        LOGGER.info('%s', msg)
    return check_result


def main():
    """
    スクリプト本体
    """
    # 接続するDBのhostを定義
    local_host = COM.MY_HOST

    # データベース（baogw）への接続情報
    connect_db()

    # testファイルの有無を確認
    if file_check('test') is True:
        LOGGER.info('testファイルが見つかったため、処理を終了します。')
        exit()

    # Primaryファイルの有無を確認
    elif file_check('primary') is True:
        LOGGER.info('primaryファイルが見つかったため、rescue未処理レコードをgw_eventsに挿入します。')

        # isertレコードの抽出、search_rescue_events()をキックし、結果をfound_recsで受け取り    
        found_recs = search_rescue_events()
        # 抽出結果のチェック
        evtnum = len(found_recs)
        if evtnum == 0:
            # 処理対象無し
            LOGGER.info('対象が見つからなかったため、処理を終了します。')
        else:
            # 処理対象有り、処理対象を順次取出し、gw_eventsへ挿入、gw_rescue_eventsの削除実施
            for fuond_rec in found_recs:
                # isert_gw_events() キック
                insert_gw_events(fuond_rec)
                # 結果 成功の場合、gw_rescue_eventsの該当レコード削除
                if insert_result == 0:
                    delete_condition = 'gw_event_id=%s',fuond_rec['gw_event_id']
                    # delete_baogw() キック
                    delete_gw_rescue_events(delete_condition)

    else:
        LOGGER.info('test、primaryファイルが見つからなかったため、rescue未処理レコードの処理状況を確認します。')
        # isertレコードの抽出、search_rescue_events()をキックし、結果をfound_recsで受け取り    
        found_recs = search_rescue_events()
        # 抽出結果のチェック
        evtnum = len(found_recs)
        if evtnum == 0:
            # 処理対象無し
            LOGGER.info('対象が見つからなかったため、処理を終了します。')
        else:
            # 処理対象有り、gw_eventsに該当レコードがあるかチェックし、あればgw_rescue_eventsの削除実施
            for fuond_rec in found_recs:
                # search_events() キック　結果のlengthチェックで該当レコードの有無をハンドリング
                search_events(fuond_rec)
                # gw_eventsにレコードが存在した場合、gw_rescue_eventsの該当レコード削除
                if check_result == 0:
                    LOGGER.info('該当レコードがgw_eventsに見つからなかったため、次回処理に繰り越します。%s',fuond_rec)
                else:
                    delete_condition = 'gw_event_id=%s',fuond_rec['gw_event_id']
                    # delete_baogw() キック
                    delete_gw_rescue_events(delete_condition)

    # データベース（baogw）のクローズ情報
    cursor.close()
    conn.close()

if __name__ == '__main__':
    try:
        main()
    except Exception as msg:
        LOGGER.critical('[End0] Stopped by an unknown error.')
 ##       LOGGER.exception(msg)
        LOGGER.info('%s', msg)

