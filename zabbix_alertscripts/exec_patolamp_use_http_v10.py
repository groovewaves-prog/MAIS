    ...
    if patolamp_ip in COM.http_patolamps:
        for attempt in range(total_retries):
            ...
            if result is True:
                ...
                LOGGER.info('%s　%s パトランプ鳴動失敗　試行回数：%d回　cause: %s' % (patolamp_kind, place_name, attempt + 1, error))
        ...
        for attempt in range(total_retries):
            try:
                ...
                LOGGER.info('%s　%s パトランプ鳴動失敗　試行回数：%d回　タイムアウトにより中断' % (patolamp_kind, place_name, attempt + 1))
                if attempt == total_retries - 1:
                    LOGGER.info('%s　%s パトランプ鳴動最終的に失敗' % (patolamp_kind, place_name))
                time.sleep(5)  #5秒待機後リトライ
            except Exception as msg:
                LOGGER.info('%s　%s パトランプ鳴動失敗　試行回数：%d回　cause: %s' % (patolamp_kind, place_name, attempt + 1, msg))
                if attempt == total_retries - 1:
                    LOGGER.error('%s　%s パトランプ鳴動最終的に失敗' % (patolamp_kind, place_name))
                time.sleep(5)  # 5秒待機後リトライ
