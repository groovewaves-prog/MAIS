...
def file_check(filename):
    ...
    """
    return os.path.exists(os.path.join(COM.FLAG_DIR, filename))


###########これより下、patolamp_param.csvに関わる処理##########

def load_patolamp_details(csv_file):
    """
    CSVを読み込む
    """
    # スクリプトが依存する「必須カラム名」のリストを定義
    # CSVのカラムを増やす場合はREQUIRED_COLUMNSの修正も行ってください。
    REQUIRED_COLUMNS = [
        'hostgroups',
        'url',
        'series',
    ...
    try:
        with open(csv_file, newline='', encoding='utf-8') as f:
            ...
            # ヘッダー（1行目）のカラム名リストを取得
            header = reader.fieldnames
            # ファイルは存在するがヘッダーすらない（空のファイル）場合
            if not header:
                LOGGER.warning(f"patolamp CSV は存在しますが、ヘッダーがありません (空のファイル): {csv_file}")
                return [] # 空のリストを返す（エラーではない）
 
            # 必須カラムがヘッダーに存在するかチェック
            missing_cols = []
            for col in REQUIRED_COLUMNS:
                if col not in header:
                    missing_cols.append(col)
            # もし足りないカラムが1つでもあれば、ValueErrorを発生させる
            if missing_cols:
                error_msg = f"パトランプCSVのヘッダーに必須カラムが不足しています: {missing_cols}"
                LOGGER.critical(error_msg)
                raise ValueError(error_msg) # ★★★ カラム名エラーを通知 ★★★
 
            # ヘッダーがOKなら、初めてデータ行をすべて読み込む
            details = list(reader)
    except FileNotFoundError as e:
        #ファイル無しエラー
        LOGGER.critical(f"パトランプCSVファイルが見つかりません: {e}")
        raise
    except Exception as e:
        # その他の予期せぬエラー（例：ファイルの権限がない、文字コードが違うなど）
        LOGGER.critical(f"patolamp CSV 読み込み失敗：%s", e)
        raise
 
    # データ行が0件だった場合
    if not details:
        LOGGER.warning(f"patolamp CSV は存在しますが、データ行がありません: {csv_file}")
    return details

# スクリプト起動時に一度だけCSVを読み込む
PATOLAMP_DETAILS = load_patolamp_details(COM.PATOLAMP_CSV_FILE)

def make_patolamp_file(dyn_file, csv_record):
    """
    パトランプフラグファイル作成処理
    csvの必要なカラムをログに出力します
    """
    fullpath = os.path.join(COM.FLAG_DIR, dyn_file)
    try:
        # フラグファイルを追記モードで開いてすぐ閉じる
        with open(fullpath, 'a'):
            pass
        # 成功時ログ
        LOGGER.info(
            "パトランプフラグ作成：%s  (hostgroups=%s, url=%s)",
            fullpath,
            csv_record.get('hostgroups', ''),
            csv_record.get('url', '')
        )
    except Exception as msg:
        # 失敗時ログ
        LOGGER.error(
            "パトランプフラグ作成失敗：%s (%s)  (hostgroups=%s, url=%s)",
            fullpath, msg,
            csv_record.get('hostgroups', ''),
            csv_record.get('url', '')
        )
...
def create_patolamp_files(hostgroups, patolamp_file):
    """
    鳴動ファイルの命名
    """
    for rec in PATOLAMP_DETAILS:
        # CSV の hostgroups 列に書かれた文字列がZabbix から返ってきた hostgroups のリストに含まれるか
        if rec['hostgroups'] in hostgroups:
            # hostgroups + '_' + patolamp_file で動的にファイル名を作る
            dyn_file = rec['hostgroups'] + '_' + patolamp_file
            make_patolamp_file(dyn_file, rec)